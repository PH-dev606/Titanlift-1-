...
const Home = ({ prs, sessions, templates }: { prs: PersonalRecord[], sessions: WorkoutSession[], templates: WorkoutTemplate[] }) => {
  ...
  return (
    <div className="p-6 pb-32 md:pl-28 pt-[max(1rem,env(safe-area-inset-top))] md:pt-10 max-w-4xl mx-auto">
      ...
      <div className={`border rounded-[2rem] p-6 mb-10 transition-all duration-500 ${activeTemplateId ? 'bg-gradient-to-br from-indigo-900/30 to-indigo-950/50 border-indigo-500/30' : 'bg-gradient-to-br from-gray-900 to-slate-950 border-gray-800'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          {templates.slice(0, 4).map(template => (
            <Link 
              ...
              className="bg-gradient-to-br from-gray-800/50 to-slate-950 border border-gray-700/50 p-5 rounded-3xl hover:border-indigo-500/50 hover:from-gray-800 hover:to-slate-900 transition-all group relative overflow-hidden shadow-lg"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link to="/scan" className="bg-gradient-to-br from-gray-900 to-slate-950 border border-gray-800 p-4 rounded-3xl flex items-center justify-between group hover:brightness-110 transition-all shadow-md">
        ...
        <Link to="/history" className="bg-gradient-to-br from-gray-900 to-slate-950 border border-gray-800 p-4 rounded-3xl flex items-center justify-between group hover:brightness-110 transition-all shadow-md">
          <div className="flex items-center gap-3">
      ...
      <div className="grid gap-3.5">
        {templates.map(template => (
          <div key={template.id} className="relative group">
            <div className={`bg-gradient-to-br from-gray-800/50 to-slate-950 border border-gray-700/50 p-5 rounded-[1.8rem] transition-all shadow-lg ${isManageMode ? 'ring-2 ring-indigo-500/20' : 'hover:border-indigo-500/50 hover:from-gray-800 hover:to-slate-900'}`}>
...
const HistoryItem = ({ session }: { session: WorkoutSession }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-slate-950 border border-gray-800 rounded-3xl overflow-hidden transition-all hover:border-gray-700 shadow-md">
      <button 
        ...
        {activeExercises.map((ex, exIdx) => {
          ...
          return (
            <div 
              ...
              className={`bg-gradient-to-br from-gray-900 to-slate-950 rounded-[1.8rem] p-3.5 border transition-all duration-300 shadow-xl relative overflow-hidden ${isFocused ? 'border-indigo-500/50 from-gray-900/95 to-slate-950/95' : 'border-gray-800'}`}
            >
              {isFocused && (
